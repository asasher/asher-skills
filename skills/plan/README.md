...
plan and borrows sign-off from the `review-loop` skill and role selection from the `staffing` skill, both by
name. Domain-neutral — not only for code.
...
- **Thin composer, no runtime.** Plan ships no server and no scripts. It authors an artifact and composes two
  siblings by plain name: `review-loop` for the sign-off gate, `staffing` for "who authors or builds."
...
`plan-contract.md` (the four gates, the threshold, the two compositions) and `authoring.md` (what a plan
...
and `staffing` (who authors/builds) — invoked by name, never imported.

## Install

`npx skills add <repo-url> --skill plan`, then invoke it on a goal. It expects the `review-loop` and
`staffing` skills present in the project (the `setup-asher-skills` installer guarantees a skill's siblings);
absent `review-loop`, plan degrades to a local review fallback, and absent `staffing`, it states the role
question rather than inventing a roster. On a fresh repo with no `docs/agents/planning.md`, the bundled
defaults stand.
...
around the composer. Rewiring `backlog` to consume this skill by name (in place of its own bundled plan step)
is a separate issue; `backlog`'s plan step is untouched by this skill.
