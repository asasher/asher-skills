...
Maquette's human sign-off gates run through the **`review-loop` skill**: a sibling root primitive invoked by
name. Maquette carries no review server of its own and imports no review-loop files.
...
revisions. This is the stable-id convention review-loop documents in its `templates/plan-skeleton.html`
and `reference/review-loop.md`.
...
Present the rendered page through the `review-loop` skill: serve it with `--kind maquette --issue <slug>`,
point the human at the printed URL, and block on await. The exact serve/await CLI, flags, and exit-code
verdicts live in the review-loop skill's `reference/scripts.md`: `serve` is `scripts/review-server.py`,
...
review-loop specifies: tailnet path proxy plus hub, or local fallback. Maquette adds no surface knowledge of
