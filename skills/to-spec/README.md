...
- **Adapted from Matt Pocock's `to-spec`, shipped as our own.** We never install an external skill — the
  template and method are adapted and vendored here with provenance.
...
to present the spec for sign-off when the user is AFK, and never a hard dependency (skipping it still yields a
valid spec). To-spec depends on no other skill.
