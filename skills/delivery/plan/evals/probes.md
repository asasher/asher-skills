...
**P11 (ac-8).** Read `skills/delivery/plan/agents/openai.yaml`. Is it well-formed per `AGENTS.md` § Conventions (the `agents/openai.yaml` rule),
...
**P14 (issue 54).** A criterion says “a second user sees the result at 1,000-file scale,” but setup records
...
- **P12 (ac-10):** PASS only if `plan` stops at the approval record and `backlog`, as caller, owns the commit,
  tracker mirror, and implementation tail. Cite `README.md` § Not in scope or `reference/plan-contract.md`
