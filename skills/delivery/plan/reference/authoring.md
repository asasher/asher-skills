...
- **Assumptions to validate** — mechanism/library/runtime claims that remain hypotheses, each with the
  cheapest falsification probe, owner, and dependent work that must wait. An empty list is explicit.
...
- **Data is part of the criterion.** Name required entities, scale and feasibility, account/tenant, fixture
  lifetime, and observation mechanism. If the environment cannot supply them, rewrite the criterion or get
  the substitute approved here—never leave verification to improvise it.
