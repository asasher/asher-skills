...
`codex exec --sandbox read-only` — with `skills/delivery/prototype/SKILL.md` in context, plus the one `reference/`
...
**P1 (ac-1).** Read `skills/delivery/prototype/SKILL.md`. List the files the skill ships under `skills/delivery/prototype/`
(SKILL, README, the `reference/` technique, `agents/openai.yaml`, `evals/probes.md`). Does the layout match
the shipped siblings `skills/system/staffing/` and `skills/system/review-loop/`?
...
`skills/system/review-loop/…` or `skills/system/staffing/…`)? How does it refer to review-loop and staffing instead? Cite
what you checked.

**P5 (ac-5).** Read `skills/delivery/prototype/reference/prototyping.md` **without** `SKILL.md`. Does it frame a
...
`.claude/skills/delivery/backlog/reference/prototype.md` and `docs/agents/prototyping.md` and run `git diff --stat`.*
...
- **P4 (ac-4):** **No file imports another skill's files** — review-loop and staffing are referenced **by
  ...
  "imports none of their files"). A grep for `skills/system/review-loop/` or `skills/system/staffing/` paths finds none.
...
- **P9 (ac-9):** **No — backlog's files are untouched.** The standalone skill carries its **own** bundled
  copy of the technique (`skills/delivery/prototype/reference/prototyping.md`); backlog keeps `reference/prototype.md`
...
**ac-4** (no `skills/system/review-loop/` or `skills/system/staffing/` path in any skill file), **ac-9** (git diff shows
