...
5. **Verdict per criterion** — exercise each criterion against that surface and record pass/fail plus the observation. When a pre-approved substitute is required, record the criterion, gap, and substitute as a **caveat**; a silent substitute is a false claim.
