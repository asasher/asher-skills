...
Target: a branch whose behavior `reference/verify.md` has confirmed against its acceptance criteria. Terminal capture step — runs once. In the build loop it runs **after adversarial review converges**, against the branch's final HEAD, so the captured proof matches what merges and is never invalidated by fixer commits; standalone it follows verify directly. Never run it on a change that has not passed verification.
