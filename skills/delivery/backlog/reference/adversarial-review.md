...
When a comment reveals a real product-semantics question, Reviewer stops without editing and sends the
question plus evidence to the issue coordinator. The coordinator rules inside the approved plan or escalates
to the session orchestrator/human. Only an explicit ruling goes to Fixer; Fixer applies it, targeted verify
reruns, and the same Reviewer receives the new HEAD. Neither Reviewer nor Fixer invents behavior.
