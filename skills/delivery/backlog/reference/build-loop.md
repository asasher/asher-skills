...
handoffs still resolve through `staffing` at their own boundaries — every role below (builder, checker, draft
authoring) resolves through the `staffing` skill (by name).
...
Liveness: this thread is bound by `reference/run.md` § Liveness contract. Terminal CI waits are the run thread's, not yours: report the PR and hand the CI watch up rather than parking here.

Tracker write discipline: on the local binding, follow `platform.md` § The local binding's write classes — this thread commits only class-2 writes (its own issue file, on its own branch); class-3 abort writes go through the run thread, the serialized main writer, never committed here. On bindings with a live tracker (GitHub), write directly via the tracker verbs.
...
2. Confirm the work-type and route. Read `docs/agents/backlog-policy.md` for this repo's label roles.
   ...
   - If orienting instead reveals unsettled *strategic* decisions — product judgment, scope, or design direction the groomed context neither settles nor explicitly delegates — hand it back the same way as **`needs-shaping`**: comment the specific open decisions, apply the role, drop the claim, and open no PR.
   - Completion criterion: the work-type is known and ready to route, or the issue is handed back as `needs-info` or `needs-shaping`.

3. Execute the branch.
   - **bug** → follow `reference/diagnose.md`.
   - **refactor** → follow `reference/refactor.md`.
   - **enhancement** → the strategic decisions arrived settled or delegated (groom's recorded `route: direct` judgment). Draft a **just-in-time tactical plan** — scoped to this ticket, inside the delegated authority, recorded in the thread and reflected in the PR body; never a human gate — then follow `reference/implement.md`. If a strategic question turns out to be open, or implementation **invalidates** an approved decision, do not settle it here: record the finding on the issue and hand it back as `needs-shaping` per step 2 — the work returns through shaping. There is no in-run planning approval gate.
   ...
   - **draft** → produce-and-review, for judgment-terminal work whose correctness is taste/fit (a memo, copy, narrative synthesis, code docs). Produce the artifact — authoring is a staffed role — then present it for sign-off through the **`review-loop`** skill (by name); the human review verdict **is** the definition of done. **Keep** the artifact: commit it to the work branch. There is no plan/implement spine and no mechanical acceptance-criteria run — see step 4.
   - Any branch — not just enhancement — may hand a blocking design question to the `prototype` skill (by name); the branch's own reference states when — and only for questions **within the issue's delegated authority**. A strategic question is a `needs-shaping` handback, never a prototype session.
   - Diagnosis is coordinator-owned and stays here; new judgment or hard-diagnosis uncertainty escalates through the recorded upward successor.
   - Completion criterion: the branch's own completion criterion is met, or the issue is handed back (`needs-info` / `needs-shaping`).
...
4. Verify behavior → follow `reference/verify.md`.
   ...
   - Delegate the whole loop to a subagent filling the **checker** role; this thread stays coordinator and takes back any failure the subagent escalates per `verify.md`'s triggers.
...
7. Capture evidence → follow `reference/evidence.md`.
   - Runs once, after the review loop ends, against the branch's final HEAD; `reference/evidence.md` carries the timing rationale. Skip if the playbook requires no evidence beyond green checks. Delegate the capture to the **checker** role.
