...
- Create an issue coordinator with a prompt, working directory, and the route `run` already selected: _<record each effect-verified native or sibling-harness verb. Examples: native agent tool with `isolation: worktree`; Claude→Codex `codex exec --cd <worktree> ...`; Codex→Claude bounded `claude -p` with a self-contained prompt and **no `--bare`**>_.
