...
- **Sibling skills** — `backlog` composes `diagnosing-bugs` (bug method), `staffing` (roster/roles/fallback), `review-loop` (presentation + interactive review), `plan` (planning + approval gate), and `prototype` (throwaway design questions) **by plain name**; setup ensures their required closure.
