...
Spreadsheet-loop's human sign-off gates run through the **`serve-via-tailnet` skill** — a sibling root primitive
invoked by name. This skill carries no review server of its own and imports no serve-via-tailnet files.
...
as one page or two, but keep ids stable across revisions. This is the stable-id convention serve-via-tailnet
documents in its `templates/plan-skeleton.html` and `reference/serve-via-tailnet.md`. Inline all styles; no external
fetches. The render is ephemeral; the markdown stays canonical.

## Serve and await

Present the rendered page through the `serve-via-tailnet` skill: serve it with
`--kind spreadsheet-loop --issue <slug>`, point the human at the printed URL, and block on await. The exact
serve/await CLI, flags, and exit-code verdicts live in serve-via-tailnet's `reference/scripts.md`: `serve` is
...
serve-via-tailnet specifies: tailnet path proxy plus hub, or local fallback. This skill adds no surface knowledge of
