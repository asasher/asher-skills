...
and are part of review-loop's bundled-reference contract — a caller invokes them from this doc without
reading the source. The loop they implement is in [review-loop](review-loop.md); the surface and hub are in
[surface-and-hub](surface-and-hub.md).

## `review-server.py`

Serves one artifact with the chrome injected, collects batched feedback into `<state>/events.jsonl`, binds
approvals to the document hash, and maintains the hub in the surface dir. Deregisters on clean exit.
...
| `--state` | `state` | run-scoped dir holding `events.jsonl`, `ledger.json`, `.await-cursor`. |
...
```json
{"url": "...", "local_url": "http://127.0.0.1:<port>/?token=...", "hub": "...", "port": <port>, "token": "..."}
```
...
- `GET …/version` — `{"hash": "<current doc hash>"}`; the chrome polls it to auto-reload on a new version.
  Requires the token.
...
```sh
# Serve; capture the printed JSON to learn the port.
