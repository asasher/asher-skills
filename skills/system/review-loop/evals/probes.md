...
**P1 (ac-1).** Read `skills/system/review-loop/SKILL.md`. Does its frontmatter identify it as a user-invocable,
artifact-agnostic review/sign-off primitive? List the files the skill ships under `skills/system/review-loop/`
...
**P9 (ac-10).** Read `skills/system/review-loop/agents/openai.yaml`. Is it well-formed per
...
- **P1 (ac-1):** Frontmatter has `name: review-loop`, `user-invocable: true`, and a `description` that reads
  ...
  **pass**. All listed files exist under `skills/system/review-loop/`. No file imports another skill's files (the
...
- **W1 (ac-10):** **No — do not block the orchestrator inline.** Spawn a **dedicated watcher subagent** whose
  ...
  **staffing decision** — staffed at the roster **Floor** (the cheapest class staffing names), resolved by
  ...
  poll the log = fail.
- **W2 (ac-3, ac-11):** There is **no fixed Codex-side model**. Read the roster Floor after applying the
  current directional reachability graph; a healthy Codex→Claude sibling route may make a Claude model
  reachable, while failure removes only that direction and triggers succession. The watcher is **staffed at
  the roster Floor**, not hardcoded to `sonnet` or `gpt-5.6-terra`. A generic **`staffing route`** of the watch task would **not** give the model you want:
  ...
  § Who holds it. Any fixed model, symmetric-failure assumption, or claim that generic `route` yields the
  cheapest = fail.
