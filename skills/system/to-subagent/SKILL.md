...
Carry the execution permissions in the dispatch configuration: the session's authorized mode and tool restrictions, with read-only access for read-only work where supported. Before a cross-harness builder starts, validate that its configured route can write a disposable probe in the supplied directory, then remove the probe. Stop on a denied capability and report the blocker within the authorized permission mode.
...
On return, check the deliverable against the completion condition. Return the report, worker resume reference when available, observed revisions, and completion or timeout status. Preserve requested deliverables unchanged; summarize only the outcome and implications in the parent's words.
