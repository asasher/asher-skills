...
1. Inspect registered worktrees, local branches, current remote history, and the ticket's ownership record. Keep the primary checkout's branch and files unchanged.
...
3. With no matching worktree, attach the ticket's existing local branch, or recover its remote branch with its history intact. Create a new branch from the supplied creation base only when neither exists.
4. Verify the working copy's branch, repository registration, and expected commits. Resolve an existing path or conflicting registration by inspecting and preserving its work before proceeding. Bring up the stack once using the playbook.
