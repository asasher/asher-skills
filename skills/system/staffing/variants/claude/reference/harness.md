...
explicit timeout it runs `codex exec` with the permission envelope the machine's staffing module records,
closes stdin, and returns raw output plus separate lifecycle status. Use worktree isolation for parallel
edits. Never use `claude -p` from Claude Code.
