...
`--bare` is not a harmless minimal mode. It forces Anthropic auth to `ANTHROPIC_API_KEY` or `apiKeyHelper`
and skips keychain reads, so the child bypasses the machine's OAuth subscription and bills as API credits —
paying twice for capacity the subscription already covers. It also skips `CLAUDE.md` auto-discovery, so the
child loads none of the target repo's agent instructions, including the staffing playbook pointer: a `--bare`
worker resolves unstaffed. Either reason alone is disqualifying.
