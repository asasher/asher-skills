...
2. **Which sibling harness CLIs are installed?** Probe `codex --version` and `claude --version`, then retain only the routes step 1 actually exercised. Presence alone is not reachability. Record the versions — they are part of what makes a recorded route trustworthy on this machine and stale on another.
...
- **Unavailable** — the probe failed, and the row captures the failure class (CLI absent, alias rejected, permission denied, timeout, effect denied) rather than a bare no. The failure class is what the successor decision reads.
