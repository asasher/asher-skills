...
1. Verify all required siblings are installed. Invoke `capture-to-inbox setup` by name when its instance is
   absent or incomplete; do not duplicate its materialization, external dependency, deployment, or smoke-test
   logic here.
...
Completion criterion: required siblings and the capture instance are effect-verified, project paths and
cadence are explicit, optional siblings are never treated as required, and an immediate rerun produces no
unapproved changes.
