...
const lensLabels: Record<string, string> = {
  ...
  Cash: "What is outstanding",
  Growth: "What may come next",
};

export function InternalDigestEmail({ bag }: { bag: CommsBag }) {
  return (
    <BaseEmail preheader={bag.preheader} eyebrow="Internal digest">
      ...
      {bag.sections.map((section, sectionIndex) => (
        <Section key={section.title} style={{ padding: "0 34px 8px" }}>
          ...
          <Text style={emailStyles.status}>{lensLabels[section.title] ?? "Internal"}</Text>
