...
- **Log every session; keep specs honest.** Scores and one line of subjective notes to `practice-log/` — the subjective line catches what the metrics miss. When a _loop design_ fails (not the learner), revise the drill spec and note what changed and why in its revision log.
- **Human checkpoints are the ground truth.** Schedule them into the mission; prepare the learner for them; recalibrate the rig and the rubrics against what they reveal.
