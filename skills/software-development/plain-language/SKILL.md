...
2. **The dictionary is the project's context files.** STE pairs a small approved general dictionary with your own technical names — `CONTEXT.md` is that technical dictionary. Use its terms exactly; do not coin synonyms for a term it already defines. A term you need that is missing goes into the dictionary through the project's dictionary-maintenance practice, not into ad-hoc prose.
