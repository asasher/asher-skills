...
A small ignored `.retro/checkpoint.json` in the primary checkout records the reviewed boundary and the pending session versions selected before discussion. An interruption resumes that selection, including on the first sweep. Worktrees share it; machines keep separate checkpoints. Dismissed findings still count as reviewed. Project issues provide shared duplicate checking and retain accepted learnings.
