...
Not for capturing ordinary work items (`capture`), not a general review of the codebase — its
...
- **Local-first.** Most friction is a repo binding problem — fixed in playbooks or captured as
  tickets via the `capture` sibling — and never leaves the machine.
...
`npx skills add github:asasher/asher-skills --skill retro`, then `retro setup` once in the consuming
repo.
