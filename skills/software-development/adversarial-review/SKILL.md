...
The brief may require stronger independence. File count never lowers risk. A standalone code-only review explicitly reports that behavior was not verified.
...
Each dispatched checking round consumes one pass, even if interrupted. Resume at the recorded next actor; an interrupted fix needs inspection, not a new budget. Every fix invalidates both verdicts. A late evidence or CI defect reopens this same loop.

At the bound, stop with findings. The owner may record a finite extension only when findings are resolving and narrowing, within the outer deadline; an extension cannot answer a product question. Confirm timed-out workers stopped before any further writer.
