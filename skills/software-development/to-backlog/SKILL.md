...
To-backlog owns one move: **get what the conversation surfaced into the tracker before it evaporates.** A working conversation — a user interview, a shaping thread, a build session — throws off items that are not its subject: a bug someone describes in passing, an enhancement idea, a follow-up nobody should have to remember. To-backlog sweeps them up, confirms the list with the user, and publishes each as a minimal ticket into the bound tracker — **work-typed but carrying no readiness role**, so the groom sweep finds and routes them. It is capture, not decomposition: N unrelated undecided things into the intake queue, not one decided thing into ordered parts. For the latter, the `to-slices` sibling.

The defining discipline is **context fidelity**: this conversation evaporates, and whoever grooms or builds the ticket later won't have it. Each ticket carries what the chat knew — that fidelity is what lets grooming route a clear item straight to ready instead of defaulting everything to shaping.
...
- **Work-typed, never readiness-labeled.** The work-type (bug / enhancement / whatever the label roles name) is recorded at capture because it is a fact the conversation knows. Readiness is a routing judgment and belongs to grooming — capture applying a readiness role would make it a second groom.
