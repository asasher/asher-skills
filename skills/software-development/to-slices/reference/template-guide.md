...
- **Title**: one line naming the capability the slice delivers, from the user's perspective. For example: "a user can save a draft and see it on reload."
...
- **Slice**: the tracer bullet, the one real thing this issue does end to end and how it demos alone. For a wide-refactor issue, name which phase it is (expand, migrate batch N, contract). Use durable prose (slicing § Durable content).
...
Keep every field at the altitude of direction so a fresh agent can pick up the work. If a field drifts into file-by-file detail, pull it back up; the implementing agent carries that detail.
...
- **Source**: the ticket, published spec, and exact approved hash. A revised source requires reconciling the split with that approval. On publication the ticket becomes the `spec` parent; every child attaches as its sub-issue and blocker.
...
- **Wide-refactor note**: if any part is sequenced expand, migrate, contract, call it out so the user confirms the phasing and granularity.
