...
6. Tear down the merged ticket's stack using the playbook, then remove its registered secondary working copy with native `git worktree remove`. Preserve dirty or unknown files; remove only known disposable residue. Keep the primary checkout and its branch unchanged throughout cleanup.
