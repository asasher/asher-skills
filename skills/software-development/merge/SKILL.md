...
Require the human's explicit selection before merging. Re-read each ticket and split parent for active shaping holds, newer unapproved revisions, and the split's bound revision. Merge only work released under the current approved authority.

For each selected PR, resolve conflicts with its intended base before verification. Inspect how changes on both sides interact, including when Git merges cleanly but the combined changes alter behavior. Read the affected changes and their linked tickets, approved specs, and acceptance criteria to establish the intended behavior. Preserve compatible approved behaviors. Record the source and reasoning for each behavioral resolution, including tradeoffs. If approved requirements conflict or leave the choice unresolved, stop the affected merge for a ruling. Re-run relevant verification and review after resolution.

For each selected PR, bases before dependents:
...
5. Reconcile dependent branches with the merged base, applying the same conflict-resolution rules before their turn. Changed dependents need renewed checks and review.
