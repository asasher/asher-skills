...
Pre-deployment probes per `docs/agents/probe-evals.md`: both executors, **`SKILL.md` + `reference/annotation-contract.md` in context**, exact-sentence citation per answer. Ambiguity flagged with a citation is valid. Key before runs.

## Scenario

A spec at `docs/specs/payouts.html` (stable element ids) needs the user's verdict; they're on their phone. Separately, a status report just needs viewing.
...
**P5 (missing ids).** A different artifact has no stable element ids. What happens when it's served annotated? Cite.
...
- **P3:** "The machine's presentation conventions (the global instruction files' presentation module) record the tailnet root, port ranges, and any reverse-proxy rules — honor them." Inventing a root = **fail**.
