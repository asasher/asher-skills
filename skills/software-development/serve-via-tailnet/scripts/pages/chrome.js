...
(function () {
  ...
  // Build client→server URLs relative to the page's mount base, so requests
  // resolve whether the page is served at "/" (loopback) or under a tailnet
  // path prefix (e.g. /asher-skills/2/review/). Strips the trailing segment of
  // the current path to get the directory the doc is served from.
  ...
  root.innerHTML =
    ...
    '      &middot; <a href="' + esc(B.hubUrl || apiUrl('hub')) + '">hub</a></div>' +
    '  </header>' +
  ...
  setInterval(function () {
    fetch(apiUrl('version') + '?token=' + encodeURIComponent(B.token)).then(function (r) { return r.json(); }).then(function (v) {
