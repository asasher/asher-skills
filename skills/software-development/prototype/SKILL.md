...
4. **Cleaned.** Park the artifact on its artifact branch (`artifact/<ticket>-<slug>`; `artifact/<slug>` when ticketless) with a `to-web` render link in the record (absent that sibling, link the branch file). Nothing throwaway ships — the build that follows rebuilds the winner or lifts the validated logic, and the branch is deleted when spent, never in this run.
