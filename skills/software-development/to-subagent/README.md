...
- **Siblings (optional, by name):** `staffing` — roster, wake-path ladder, and succession and route-loss records on worker death; absent it, the subagent runs on the dispatching session's model and effort. `watch-until` — the bounded watch on the durable surface behind a background wait; absent it, the parent polls the durable surface itself, bounded.
