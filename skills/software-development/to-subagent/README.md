...
deliverable, uses the workflow-supplied directory exactly, and wires a wake path so the dispatcher learns
of completion without polling. Direct invocation creates a worktree only on an explicit isolation request;
composing workflows retain worktree policy and cleanup ownership. One call
