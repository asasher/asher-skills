...
- **Siblings (optional, by name)** — `to-web` (deploys the render; the projection links the returned hash-keyed URL — absent it, link the branch file and say the render was not deployed), `plain-language` (the communication standard — absent it, write plainly and say the standard was not loaded).
- **Project playbooks** — the **tracker and branch bindings** in `docs/agents/platform.md` (how a ticket is read, commented, and created; the `artifact/` branch convention).
