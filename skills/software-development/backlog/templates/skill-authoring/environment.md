...
>
> Machine facts follow the `backlog` skill's `reference/machine-facts.md` discipline: record the probe
> command, not its result; a fact only this machine can answer lives in the gitignored
> `docs/agents/local/environment.md` overlay, regenerated by the owning setup; a tracked probe result
> sits under a `machine-record` stamp.
