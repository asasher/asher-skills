...
**Closure** — the change request's closing reference (`Closes #N`) closes the ticket on merge to the base branch; there is no post-build label by default. Stacked slices reach closure through `delivered` above: the slice merge sets the label, the promotion merge closes the ticket.
...
- A claim past its deadline is not abandonment by itself: `backlog status` derives abandonment only when the thread is also not observably alive — liveness decides, not the clock alone.
- The **quiet horizon** (§ Building hygiene) survives only as the backstop for claims that predate deadlines.
