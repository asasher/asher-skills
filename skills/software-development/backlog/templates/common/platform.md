...
> Read by the `to-web` sibling, and through it by `prove-your-work` (evidence media), `shape` (spec renders), `prototype`, and `research` (preview-deployed renders). Media is never committed to the repo; the bucket is its permanent home, and callers embed by URL. Visibility is fixed: **public with unguessable keys** — content-addressed or random keys, immutable URLs, no listing.
