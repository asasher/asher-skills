...
>
> Machine facts follow the `backlog` skill's `reference/machine-facts.md` discipline, at accretion and
> at setup alike: record the probe command, not its result; a fact only this machine can answer lives
> in the gitignored `docs/agents/local/codebase.md` overlay, regenerated by the owning setup; a
> tracked probe result sits under a `machine-record` stamp.
...
- Interpreter/runner facts are verify-at-use: state the requirement the code imposes ("Python
  3.10+") and let the gate run itself be the probe — never record the observed version. An expensive
  verified behavior — a sandbox refusal class, a runner quirk proven live — is recorded under a
