...
repo — a stale or missing record is the same drift, fixed by re-running the owning setup before
dispatch. For each ticket: mark it building per the label roles — a dispatched ticket must never
