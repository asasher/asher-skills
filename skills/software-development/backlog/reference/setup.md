...
2. **Certification** → invoke `agent-ready-codebase` and record its demonstrated results in the playbook's § Agent-readiness. Include shared singletons and gaps. Each gap is groomable work; re-certify when setup changes. Record observed merge protection; its absence blocks neither build readiness nor merging.
