...
- **The dispatch declaration** — every spawn opens with a declaration in the transcript, posted before the call goes out: "Dispatching <work> — model <X>, effort <Y>, harness <Z>, deadline <absolute time>." A statement, never a question: the human can interrupt, nobody must approve, and the transcript is the audit trail. User-facing text — the declaration and the report — follows the `writing-for-humans` sibling; absent it, write plainly and say the standard was not loaded.
...
- **Bundled scripts** — exercised from inside the route files, not named elsewhere in this file: `scripts/t3-thread.py` (local T3 HTTP dispatch, used by `reference/t3.md`) and `scripts/name-codex-thread.py` (Codex post-creation naming, used by the Codex routes).
