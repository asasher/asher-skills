...
- **The dispatch declaration** — every spawn opens with a declaration in the transcript, posted before the call goes out: "Dispatching <work> — model <X>, effort <Y>, harness <Z>, deadline <absolute time>." A statement, never a question: the human can interrupt, nobody must approve, and the transcript is the audit trail. User-facing text — the declaration and the report — follows the `plain-language` sibling; absent it, write plainly and say the standard was not loaded.
