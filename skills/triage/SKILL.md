...
- References orchestrate; playbooks instruct. A subcommand reads its own step playbook (and `environment.md` when it touches the app) for the whole technique. The shipped playbooks are self-contained — no external skill is required — though `setup` may fold a repo's own practice into them.
