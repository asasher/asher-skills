...
2. **Walking skeleton row** — directly under each activity, its user _steps_ as smaller cards (`rx=4`, 32px tall, `paper-2` fill, `ink` stroke), Geist sans 12px. One or two per activity; reserve room for two so every activity's row bottom aligns even when it only has one step.
