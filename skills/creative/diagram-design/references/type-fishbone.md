...
- **Evidence status.** The investigation may have zero, one, or several confirmed causes. Label causes as confirmed, suspected, or ruled out according to the evidence, at the bone or sub-cause where that evidence applies. State when none is confirmed. Use text or symbols so status remains clear without color. Accent may emphasize up to two focal elements; it does not determine cause count or confirmation.
...
**Sub-cause ticks** sit at fractions `m/6` along the bone (`m = 2, 4` for two ticks; `m = 3` for one):
...
| Max accent elements | 2 focal elements, independent of evidence status |
...
- **Changing the findings to fit the style.** Preserve every confirmed cause. Split for readability when needed, keeping an overview of the same effect and all causes; never invent, merge, or discard causes to satisfy an accent budget.
