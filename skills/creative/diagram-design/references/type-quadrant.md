...
A **layout variant** of the standard quadrant — same house skin (black paper, dot pattern, Instrument Serif title, Geist mono eyebrows, accent focal rule). The grammar shifts: axes hold a **range** rather than a measurement; cells hold **named scenarios** rather than positioned items.
...
| Focal accent | accent on one _item_ | accent on one _quadrant_ — tinted bg + accent stroke + accent corner tag |
...
- **Axis lines**: `ink` (`#ededed`), `stroke-width: 1.2`, `marker-start` + `marker-end` both pointing outward.
- **Focal quadrant tint**: `rgba(82,168,255,0.04)` full rect behind the focal cell.
...
- Plain white background — the house paper + dot pattern is load-bearing across the skill; dropping it to "look consultant" turns the diagram generic.
