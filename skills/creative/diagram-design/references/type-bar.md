...
- **Y-axis gridlines:** 4–6 horizontal lines at regular intervals. Stroke `rgba(237,237,237,0.08)` (very faint), 0.8px. X-axis baseline at `rgba(237,237,237,0.25)`, 1px.
...
- **Gridlines:** vertical at each tick, `rgba(237,237,237,0.08)` 0.8px, spanning y 56→408. At the domain floor the axis line replaces the gridline rather than doubling it: `rgba(237,237,237,0.25)` 1px, y 40→420.
...
Verify the value domain and finite coordinates against the actual data, including all-zero or zero-touching data where present. Check that marks meet 3:1 contrast against the composited background and that every drawn position agrees with its printed value. Use available calculation and browser tools for these checks.
