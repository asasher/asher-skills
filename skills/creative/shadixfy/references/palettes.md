# shadcn/ui base palettes

Verbatim shadcn/ui (v3) neutral base palettes, in `H S% L%` form, dropped straight into `:root` and `.dark`, with `--radius: 0.5rem`. Wire Tailwind to them with `hsl(var(--token))`. Copy exactly one base — Zinc (default), Neutral (pure gray), or Stone (warm gray) — and change only the semantic accent tokens if the product needs color.
