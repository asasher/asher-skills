...
The profile is the state of the whole project: everything else — assets, scouting, scoring, drafts — reads from it. It is written in the person's own words wherever possible, and every claim carries a confidence mark.
...
5. **Search parameters** — target roles and fields, location/remote and visa facts, compensation floor, dealbreakers, urgency and runway. This is what opportunity scoring scores against; without it the list has no ranking basis.

Deliberately absent: referral networks, contact lists, relationship tracking. Warmth exists only as a per-opportunity field ("is there an insider, or a path to one?") in `opportunities.json`.
...
The search's activity exists partly to move claims rightward. When advice depends on a claim's mark, say so and propose the cheapest way to upgrade it.
...
Mine a CV, LinkedIn, performance review, or reference letter for **episodes, choices, and repeated themes — not adjectives**. "Great communicator" in a review is _reported_; the episode it cites is _evidenced_.
