...
Coverage map, not script. Open from the record: read the CV/LinkedIn/portfolio first, play back what you inferred, and ask what you got wrong.
...
- **Direction is open** — sketch three genuinely different five-year versions of their work life: the current path done well, the adjacent path if that vanished, and the path if money and others' judgment didn't matter. Ask which parts of each pull at them; the pull pattern is profile data. Plans must diverge in kind, not salary band.
- **They can't or won't leave yet** — redesign before resigning: which two tasks nearest their energizers could grow, which top drainer could shrink, batch, or be renegotiated? Frame each move as a small two-week experiment with a visible signal. If the real problem is structural — indecent conditions, a blocking manager, values violation — say so plainly and shift to the search; never reframe structural harm as mindset.
- **A hypothesis needs testing** — before betting on a direction, test it at the cheapest rung that answers it: read the best insider accounts → talk to two or three people living that path → try a small real slice of the work. One named assumption per test, a time box, and a signal you'd observe if it's true. Debrief moves profile claims to _tested_.
- **Positioning is vague** — sharpen the intersection: what can they explain unusually well because they lived it, and who immediately cares? One sentence into the profile; it should feel too narrow — that's what makes it hard to copy.

## Output

`goodwork/PROFILE.md` written per [profile.md](profile.md) with confidence marks on every claim, the board regenerated, and one concrete suggested next step: `assets` if the outward surface is stale, `scout` if the parameters are settled, another session if load-bearing sections are still thin.
