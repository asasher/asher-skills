...
One self-contained HTML page generated from `PROFILE.md` and `opportunities.json`. It is a projection: regenerated whole, never hand-edited, never a source of truth, and never served — no server, no JavaScript state, no external fetches (inline styles, system fonts).
...
Prefer the harness's native presentation surface — an artifact, a rendered preview, a hosted page — whichever this session actually has. Absent one, say plainly: "I've updated your board — open `goodwork/board.html` in your browser." Never stand up a server for it.
