...
2. **Built and handed over** — the prototype runs from one command (or one URL), surfaces its state, and is presented through the surface in `docs/agents/environment.md` § Presenting (contract: `reference/presenting.md`), its URL in the pause message, so the human can drive it from wherever they are. Extend it on request; prototypes evolve.
