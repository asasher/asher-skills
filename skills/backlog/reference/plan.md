...
4. **Approved** — the rendered plan was presented through the surface in `docs/agents/environment.md` § Presenting (contract: `reference/presenting.md`, including its review loop), the pause message carried its URL and the hub URL, and a human approved it — as-is or after revision rounds in which every annotation received a ledger disposition — before any implementation begins. The approve event (verdict, content hash, timestamp) is the approval record. If approval changes scope, update the plan first.
