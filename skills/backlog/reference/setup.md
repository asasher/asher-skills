...
Prepares a repo for the loop — and reconciles a repo an earlier version already prepared. It inventories any prior installation, scaffolds or upgrades the project playbooks, binds the platforms (tracker, change review, version control, harness) and provisions the tracker's role labels, decides parallelism (preparing worktree isolation when parallel), identifies the seed and check commands, confirms app access, sets the presentation surface, installs the model-picking guidance into the repo's memory files, and verifies the repo is actually ready. Safe to re-run at any time: reconciliation never overwrites repo practice.

## Steps

1. Inventory the installation.
   - List `docs/agents/`: which playbooks exist, which of this skill's `templates/*.md` lack a counterpart, and each playbook's version stamp — the `<!-- backlog-templates: ... -->` comment on line 1. Compare stamps against this skill's `templates/VERSION`. Also check the repo's `AGENTS.md` and `CLAUDE.md` for `<!-- backlog-section: ... -->` markers and their stamps — the installed sections reconcile on the same schedule.
...
8. Install the model-picking guidance.
   - Probe what this machine can actually staff: which models the harness can spawn, and whether the Codex CLI is installed and authenticated (`codex --version`; a cheap `codex exec -s read-only --skip-git-repo-check` smoke if in doubt). List only what is real.
   - Install `templates/sections/picking-models.md` — the rankings table, routing rules, and CLI mechanics — into the repo's `AGENTS.md`, wrapped in section markers stamped from `templates/VERSION`: `<!-- backlog-section: picking-models <version> -->` … `<!-- /backlog-section: picking-models -->`. Append to an existing `AGENTS.md` or create one. Reconcile like a playbook: content inside markers that matches an older shipped default updates to current; user-edited content is repo practice — leave it, flagging only contract changes. Drop table rows and rules for models nothing on this machine can reach, and report each drop.
   - Install `templates/sections/claude-md-overlay.md` into `CLAUDE.md` the same way (marker name `picking-models-claude`), and ensure `CLAUDE.md` references `AGENTS.md` (e.g. an `@AGENTS.md` import) so Claude sessions read the shared base; Codex reads `AGENTS.md` natively. A harness with a different memory-file convention gets the same two layers in its own files.
   - These sections serve every session in the repo — workflows and ad-hoc work included — not just this skill's loop; `AGENTS.md` and `CLAUDE.md` are user-owned files, so touch nothing outside the markers.
   - Completion criterion: both sections installed and stamped, listing only reachable models, and `CLAUDE.md` references `AGENTS.md`.

9. Write the contracts into the playbooks.
   - Record the results in `docs/agents/environment.md`: dev-stack startup, isolation regime and how to bring up an isolated stack, seed regime and command, auth/session minting, the app drivers and evidence-capture tooling from the access audit, the presentation surface, the parallelism verdict with its serialized exception lane, and the compiled model-staffing roster — the `AGENTS.md` § Picking models rules crossed with this repo's surfaces and step 8's reachability probe (orchestrator / builder by surface / checker / floor, per harness the loop runs from, plus the succession line — per `reference/staffing.md`). The platform bindings live in `platform.md` from step 3. Record the check commands in `verifying.md` and the repo's evidence expectations — including the presentation contract for the bound review surface — in `evidence.md`.
...
11. Verify readiness and confirm.
   ...
   - Report a readiness checklist: playbooks present and current-stamped; platform bindings verified per verb; tracker roles provisioned; dependency convention set; seed and checks identified and real; app driver, evidence capture, and auth confirmed per surface (and smoke-tested if run); presentation surface working; model-picking sections installed and stamped in `AGENTS.md`/`CLAUDE.md` with the compiled roster resolving; parallelism verdict and exception lane; playbooks tailored to house practice where the repo has one; migrations applied and conflicts resolved.
