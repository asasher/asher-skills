...
Staffing: diagnosis is **orchestrator** work — it runs in the invoking thread; once the cause is named, the fix commit may be delegated to the **builder** for its surface, resolved by the `staffing` skill (by name).
...
If the fix itself demands a contested design decision — more than one plausible rework, expensive to reverse — hand that question to the `prototype` skill (by name) before committing to one, and record the answer on the issue.
