#!/usr/bin/env python3
"""Backlog review server; stdlib only. Contract: reference/presenting.md § Review loop.

Serves ONE document (a plan or a prototype answer sheet) with the annotation
...
Sweep mode (`--sweep --surface <dir>`): probe every registry entry's URL, drop
the dead ones, regenerate the index, exit. Run by setup's health check.
"""
...
<body>
<main>
<h1>Awaiting review <small>· backlog</small></h1>
{rows}
...
def regen_index(surface: Path) -> None:
    registry = load_json(surface / "registry.json", {"entries": {}})
    ...
    atomic_write(surface / "index.html", INDEX_TEMPLATE.format(rows=rows, generated=html.escape(now_iso())))


def update_registry(app: dict, **changes) -> None:
    path = app["surface"] / "registry.json"
    ...
    atomic_write(path, json.dumps(registry, indent=1, ensure_ascii=False))
    regen_index(app["surface"])


def remove_registry(app: dict) -> None:
    path = app["surface"] / "registry.json"
    registry = load_json(path, {"entries": {}})
    if registry.get("entries", {}).pop(app["entry_id"], None) is not None:
        atomic_write(path, json.dumps(registry, indent=1, ensure_ascii=False))
    regen_index(app["surface"])
...
class ReviewHandler(BaseHTTPRequestHandler):
    server_version = "BacklogReview/1"
    ...
    def do_GET(self):
        ...
        if parsed.path == "/":
            ...
            self._json(HTTPStatus.OK, {"hash": self.check_doc()})
        else:
...
def main() -> int:
    ...
    parser.add_argument("--kind", default="plan", choices=("plan", "prototype"))
    parser.add_argument("--state", default="state", help="run-scoped dir: events.jsonl, ledger.json")
    ...
    if not args.doc or not args.title:
        print("--doc and --title are required (unless --sweep)", file=sys.stderr)
        return 2
    ...
    entry_id = f"{args.issue}-{doc.stem}"
    server.app = {
    ...
    print(json.dumps({"url": public_url, "local_url": local_url, "hub": hub_url, "port": port, "token": token}), flush=True)
