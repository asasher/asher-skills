...
assert "setup: skill committed into the repo (clone-gets-skill)" "gitq '$A' diff --cached --name-only | grep -q '.claude/skills/fair-deal/SKILL.md'"
...
assert "join: B got the skill on clone" "[ -f '$B/.claude/skills/fair-deal/SKILL.md' ]"
