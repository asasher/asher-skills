...
- `outside-in` — reference: the shaping decision-ordering standard — users → experience → system behavior → implementation; the seam as a named handoff point; recommend-don't-ask in the implementation register; per-register blessing. Cited by `shape`, `to-spec`, and `backlog groom`.
