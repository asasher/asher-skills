...
## 2026-09-16: preserve approved behavior when resolving merges

`merge` resolves conflicts for every selected PR before verification, including the first PR and dependents changed by earlier merges. It checks both sides against linked tickets, approved specs, and acceptance criteria, including behavioral interactions when Git merges cleanly. Resolutions preserve compatible approved behavior and record their sources, reasoning, and tradeoffs. Conflicting or unresolved approved requirements stop the affected merge for a ruling; changed resolutions require renewed verification and review. Bases still merge before dependents.

Reconcile: refresh `merge`, including its provenance README. No setup, dependency, or install-set changes.

## 2026-09-15: script-generated UUID4 upload keys

`to-web` requires a full UUID4 generated by its bundled Python script using operating-system randomness for each new upload key. Agents use the script's output unchanged; UUID generation failure blocks the upload.

Reconcile: refresh `to-web`, including `scripts/generate-uuid.py`. Python 3 must be available when publishing. No setup or install-set changes.
