...
- Usage surface — read by `backlog build`'s quota gate (`backlog-policy.md` § Quota awareness); both verbs verified live at binding time:
  - Claude Code: `claude -p /usage </dev/null` — prints each window as a used percentage with its reset time (current session, current week all-models, current week per-model). Those window lines are the gate's input; the "what's contributing" breakdown printed below them is local-approximate and is not.
  - Codex: every `codex exec` turn records a `token_count` event carrying `rate_limits` — each window's `used_percent` and `window_minutes` — in its session rollout, `~/.codex/sessions/<yyyy>/<mm>/<dd>/rollout-*-<thread-id>.jsonl`, thread id from the run's `thread.started` event in the `--json` stream. Dispatched Codex work therefore reports its windows as a side effect of running; a fresh read outside a dispatched turn is one minimal bounded probe turn — `environment.md`'s credential-preflight `codex exec` probe already is one, so the per-run preflight yields the run's opening Codex reading for free.
