...
- Script check: for any changed stdlib-Python script, `python3 -m py_compile <script>` then drive it directly (`--help`, `--sweep`, the real paths) on this machine's Python 3.14. A script _refactor_ is behavior-preserving only if the same driven paths produce the same output.
...
- Gate interpreter: bare `python3` — homebrew 3.14 on this machine, hashlib healthy across algorithms (verified live 2026-07-27, asher-skills#128). Don't reinstate the old `PATH=/usr/bin:$PATH` prefix on these gates: it pinned Apple's system python around a since-fixed homebrew hashlib breakage (`catalog.py` hashes with sha256; the record is in `relay-workspace/*/grading.md`); the worktree sandbox that builds run in refuses that literal shape on a static-verification check, though plain env-assignment prefixes such as `FOO=1 python3` run fine; and `/usr/bin/python3` is 3.9.6, older than the 3.14 the script check above exercises.
