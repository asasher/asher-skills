...
- Present the plan through the **`review-loop`** skill by name (its `reference/review-loop.md`): serve it with review-loop's review server, end the pause message with the plan URL and the hub URL, and await the verdict. **Hold the watch on a dedicated staffing-resolved watcher subagent, not the orchestrator inline** — the delegated-watch contract is `skills/system/review-loop/reference/watch.md`. Who approves: Asher (the human driving the session).
