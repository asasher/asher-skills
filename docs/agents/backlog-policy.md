...
- **Usage threshold**: **75%** — this repo's standing choice, set after the 2026-07-30 overnight run took the ChatGPT weekly window from 31% to 100% and tripped the Claude session limit an hour in, killing cross-harness implementers mid-flight and leaving the week's interactive shaping and grooming without headroom (asher-skills#175). It applies to **every window any bound usage surface reports** (session, weekly, per-model weekly, on either harness): one tripped window gates claiming, whichever it is. Raising it is a playbook edit, not a run flag.
