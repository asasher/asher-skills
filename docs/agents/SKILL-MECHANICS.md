...
Skill's frontmatter carries no defaults. A key restating harness default behavior (`user-invocable: true`, an empty `requires`/`optional` list) is dropped; `metadata` holds only what something consumes — `requires`/`optional` (the dependency record), `setup`, `external`.
