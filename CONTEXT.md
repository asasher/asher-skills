...
**Repo context files**: The repo-owned strategic context a shaping session reads at intake, three files with three owners: `CONTEXT.md` (the domain glossary — `domain-modeling`), `PRODUCT.md` (strategy and the user-type roster — the `experience-first` standard), `DESIGN.md` (the visual system in the open DESIGN.md format — the `bare-minimum-design` baseline). Each owner ships the file's skeleton, creates it lazily on first need, and registers it in the instruction file's `## Context documents` index at creation — so the instruction-file footprint stays one line per file. They carry domain and direction; playbooks under `docs/agents/` carry operational bindings — a shaping decision consults the former, a build step the latter.
