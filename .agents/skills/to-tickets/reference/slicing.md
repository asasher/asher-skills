...
- **An authority boundary** — what the executor may decide (feeding its route judgment and just-in-time
  tactical plan) vs what is settled and must not be re-decided.
...
The readiness decision and who makes it are recorded in `backlog-policy.md` § Readiness decision. **Note
the option** to apply readiness on approval — the quiz *is* a human confirmation, so a user who wants it
may bless the tickets on the spot — but absent that request, leave readiness unset.
