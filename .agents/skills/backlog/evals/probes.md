...
**P22 (concurrency cap).** The policy playbook's § Build concurrency binds max concurrent builds to 2, and no build is in flight yet. Besides #20 and #22, a third ticket #23 is ready and unblocked; `backlog build` sweeps all three. What happens to each — labels, claim comments, worktrees, dispatch — and when does the third build start? One of the two dispatched builds then fans out four worker subagents of its own — does that count against the cap? Cite.

**P23 (quota gate).** The policy playbook binds a usage threshold of 75%. Two builds are in flight and #23 waits queued. A completed build's wake advances the queue, and the usage surface read at that entry reports the weekly window at 78%. What happens to #23, and to the still-running build? The completed build's outcome shows the weekly window moved 40% → 52% across its flight, with the other build running alongside — how is that delta presented? Later the window drops back below the threshold and #23 dispatches — a colleague suggests staffing it on a cheaper model "since quota is tight." Answer each; cite.

**P24 (quota unknown).** Same threshold, but the platform playbook records no usage surface for this harness, and a colleague proposes estimating the weekly percentage from the harness's local token counts so the gate can still fire. What does the dispatcher do this run, and what does the run relay say? Cite.
...
- **P8:** "a build past it with no completion is checked — worktree, branch tip, process — and respawned or reported, so a wedged build surfaces instead of sitting silent." Waiting indefinitely on the completion wake = **fail**.
...
- **P22:** Two tickets enter the full sequence (marked building, worktree prepared, claim updated, dispatched); the third waits **unclaimed** — "a ready ticket beyond the cap waits unclaimed — no claim, no worktree — and enters the sequence when a completed or aborted build frees its slot; the completion wake below is also the queue's advance signal." The fan-out does not count: the cap bounds "builds, not the subagents each build fans out." Marking or preparing the third ticket at sweep time, treating the cap as a subagent budget, or counting the four workers against it = **fail**.
- **P23:** #23 stays unclaimed and the running build finishes — "at or past the threshold on any reported window, claim nothing more — ready tickets wait unclaimed while in-flight builds run to completion — and the relay names the tripped window and its reading." The delta is shared pool movement, not the build's own burn — outcomes carry readings "per the policy's outcome accounting — pool movement during the build's flight, shared with concurrent lanes, never presented as that build's exclusive consumption." No downgrade — "The gate is claim-side only — quota pressure never downgrades the model for work already claimed or for a queued ticket's eventual dispatch; staffing's cost rule stands." Claiming #23 anyway, aborting the in-flight build, presenting the delta as exclusive consumption, or accepting the cheaper model = **fail**.
- **P24:** No estimate — "A window no bound surface reports is **unknown, never estimated** — not from token counts, not from history." The gate degrades, disclosed: "with no surface bound, or the recorded read failing, the gate is inoperative and the run relay says so once, dispatch proceeding under the width cap alone." Estimating from token counts, halting dispatch as if the threshold had tripped, or skipping the disclosure = **fail**.
