...
2. **Built and handed over** — the prototype runs from one command (or one URL), surfaces its state, and is presented through the surface in `docs/agents/environment.md` § Presenting (contract: `reference/presenting.md`), its URL and the hub URL in the pause message, so the human can drive it from wherever they are. When the deliverable is a document (a UI variant sheet, a logic write-up), serve it through the review loop so the answer arrives as annotated feedback rather than prose in chat. Extend it on request; prototypes evolve.
