...
The bundled, authoritative technique for the `prototype` skill. This is what makes the skill usable
anywhere — a codebase, a document, a design surface, with or without a project playbook. Repo-specific
placement (task runner, where throwaway artifacts live, the component library variants must use) lives in the
project playbook `docs/agents/prototyping.md`; read it for the "where," read this for the "how." Absent a
playbook, fall back to the defaults called out below rather than improvising repo structure.
...
has. Throwaway from day one: keep the answer, delete the artifact.
...
- **One command to run, or one URL to open.** No one should have to remember a path. For code, wire it into
  the task runner; for a document, it is the file that opens.
...
- **Non-code:** the same discipline without a program. A **hand-driven state table** — a document that lists
  ...
  **scenario run** where you narrate the model's response to a script of real inputs and record where it
  surprises you. The point is identical: drive the idea through cases until it either holds or breaks.
...
- **Non-code (a document, layout, slide, or doc UI):** the variants are **structurally different drafts of
  ...
  switcher; either way the human flips between real alternatives, not descriptions of them.
...
artifact / variant sheet so the answer arrives as annotated feedback rather than prose in chat, and end the
pause with its URL and the hub URL. A live interactive prototype (a terminal behavior app, a running page) is
driven directly, but its URL is still announced on the presentation surface so the human can reach it. Where
...
Framing the question and reading the answer is orchestrator work and stays with the invoking thread. Building
the artifact — the reducer and shell, the variants, the draft sheets — is dispatched to the builder the
**`staffing`** skill resolves (by name): mechanical build-out to the pinned mechanical model, anything with
taste weight (a UI variant, a rendered document) to a taste-ranked model. Don't hand-wave the routing; ask
staffing.
...
- **Capture the answer** — which option won and why — into the consuming plan when planning invoked this;
  ...
  variant (a screenshot of a UI, the rendered sheet of a document) and embed them with the winner marked, so
  the decision record outlives the prototype. The prototype itself is never the record.
- **Then delete.** Losing variants and the switcher go; a winning variant is rebuilt properly (it was written
  under prototype constraints), and a validated behavior module is lifted into real code. Nothing throwaway
  stays.
