...
`claude -p --model <verified-alias> '<self-contained prompt>' </dev/null` with the permission envelope the
machine's staffing module records, never adds `--bare`, and returns raw output plus separate lifecycle status.
