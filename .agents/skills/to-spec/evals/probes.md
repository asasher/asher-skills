...
- **P6:** "Split this into tickets" — either statement of the rule passes: synthesis.md's "Never call the downstream unit an "issue" — that's one tracker's word", or SKILL.md's "Say "spec" and "ticket" — never GitHub-specific "issue."" (both are in context and state the same rule). Keeping "issues" = **fail**.
- **P7:** A diagram — "Every spec **opens with a diagram** of the moving parts — before any prose"; "written as a fenced `mermaid` block". Prose first, or no diagram without saying why, = **fail**.
