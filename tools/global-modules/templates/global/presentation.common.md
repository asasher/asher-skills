...
<!-- Seed template: setup fills the <owner> and <tailnet-root> placeholders from this machine's facts
     before staging; never install the placeholders verbatim. -->

- Write HTML deliverables—plans, reports, prototypes, and review sheets—as local files in the repo or workspace.
- When <owner> is at this machine, open the local file. When they are remote, use the project’s `docs/agents/` tailnet surface. Always open whatever is presented; never only print a link.
- Disk is the default presentation surface. Do not use ChatGPT canvas/site, Claude artifacts, or another harness-hosted page. Publish to a cloud surface only when <owner> explicitly asks.
- This machine’s tailnet root is `<tailnet-root>`; Funnel stays off.
