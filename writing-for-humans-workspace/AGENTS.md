...
Snapshot both skill sources, the known topic, and participant settings with `prepare`. It creates a standalone temporary repo outside the authoring repo, with the two complete local skill packages, including their invocation settings. Keep all iteration metadata, source paths, feedback, and analysis in the runner workspace. Use an ordinary task title and branch, and a prompt containing only the topic. The participant must discover the skills through the normal harness; do not add skill mentions, use directives, or an AGENTS.md that forces activation. Use `start` to launch through `to-thread` in the user's T3 harness. Verify the launch with `status`, inspect the first assistant message for provider errors and verify the provider's actual working directory, then give the human the thread name. Keep the participant's context free of eval diagnoses, prior feedback, and desired writing changes. Observe whether the harness lists the skills and whether the model reads them; preserve the evidence separately from judgments about writing quality. Do not infer activation from a pleasant reply or ask the participant to report on its skill usage. The human supplies every follow-up message.
