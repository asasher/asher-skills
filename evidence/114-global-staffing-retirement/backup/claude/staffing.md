...
| Codex CLI (sibling) | none verified — `codex exec` children are bounded, untracked | gpt-5.6-terra (cron: terra) |

Hold any out-of-band wait (review verdicts, merge watches) on the top verified row for the harness running the wait: on Claude Code, a tracked background process whose verdict-coded exit wakes the session — no watcher model at all.
